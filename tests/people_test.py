...
def test_read_all_has_jonathan(logger):
    """
    Test on People GET API, we get a user named jonathan in the list of people
    """
    response = client.read_all_persons()
    
    assert_that(response.status_code).is_equal_to(requests.codes.ok)
    logger.info("User successfully read")
    assert_people_have_person_with_first_name(response, first_name='Jonathan')
